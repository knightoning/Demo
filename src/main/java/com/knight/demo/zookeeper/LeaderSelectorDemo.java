...
/**
 * 本示例创建了10个LeaderSelector并对起添加监听，当被选为leader之后，调用takeLeadership方法进行业务逻辑处理，处理完成即释放领导权。
 * 其中autoRequeue()方法的调用确保此实例在释放领导权后还可能获得领导权。
   ...
   推荐的做法是，如果发生SUSPENDED或者LOST连接问题，最好直接抛CancelLeadershipException，此时，
     leaderSelector实例会尝试中断并且取消正在执行takeLeadership（）方法的线程。 建议扩展LeaderSelectorListenerAdapter,
      a  LeaderSelectorListenerAdapter中已经提供了推荐的处理方式 。

 * @author hujie
 * @version Id: LeaderSelectorDemo.java, v 0.1 2018/2/5 15:18 hujie Exp $$
 */
